...
<head>
	<title> updateContactPage.html </title>
...
<body>
	<div class="content_page">
		<div class="main">
			<fieldset>
				<legend style="brder:1px solid red;">修改联系方式</legend>
				<form action="${pageContext.request.contextPath}/lxwm/updateContact.do" method="post" id="updateForm">
					<label>ID：</label><input type="text" name="contactSub.id" value="${subContact.id}" readonly="readonly" title="id不允许被修改" id="id"/>
					<label>分类：</label><input type="text" value="${subContact.category}" readonly="readonly" title="分类不允许被修改"/>
					<label>标题：</label><input type="text" name="contactSub.title" value="${subContact.contactSub.title}"  title="分类不允许被修改"/>
					<label style="float:left">简介：</label><textarea name="contactSub.content" rows="5" style="float:left;margin-top:11px;" required id="about">${subContact.contactSub.content}</textarea>
					<div class="cz">
