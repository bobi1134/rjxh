...
<head>
	<title> updateIntroducePage.html </title>
...
<body>
	<div class="content_page">
		<div class="main">
			<fieldset>
				<legend style="brder:1px solid red;">协会简介基本信息修改</legend>
				<form action="${pageContext.request.contextPath}/xhjj/updateBaseInfo.do" method="post" id="updateForm">
					<label>ID：</label><input type="text" name="introduce.id" value="${introduce.id}" readonly="readonly" title="id不允许被修改" id="id"/>
					<label>标题：</label><input type="text" name="introduce.title" value="${introduce.title}" id="title" required/>
					<label style="float:left">内容：</label><textarea name="introduce.content" rows="5" style="float:left;margin-top:11px;" required>${introduce.content}</textarea>
					<div class="cz">
